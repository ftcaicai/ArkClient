...
public class AsPlayerState_Run_Npc : AsBaseFsmState<AsPlayerFsm.ePlayerFsmStateType, AsPlayerFsm>
{
	Msg_MoveBase m_MoveInfo;
	
	public AsPlayerState_Run_Npc(AsPlayerFsm _fsm) : base(AsPlayerFsm.ePlayerFsmStateType.RUN_NPC, _fsm)
	{
...
//				m_OwnerFsm.transform.position, m_MoveInfo.GetTargetPosition(m_OwnerFsm.transform.position));
	...
	void CheckDistance()
	{
		...
		if( eEntityType.NPC == m_OwnerFsm.Target.EntityType )
		{
			AsNpcEntity _npcEntity = m_OwnerFsm.Target as AsNpcEntity;
			if( null != _npcEntity )
			{
