...
{
	#region - %AS -
	public List<IUIListObject> GetItems() { return items; }

	public bool IsScrolling { get { return isScrolling; } }
	
	public bool LockScroll = false;   
	
	public bool lockWhenScrolling = false;
	...
	public bool IsShowingLastItem()
    {
       ...
       // return items[lastIdx - 1].gameObject.active;// && !items[lastIdx - 1].Clipped;

		if (items.Count == 1)
			return true;

		return !items[lastIdx - 1].Clipped && items[lastIdx -1].gameObject.activeSelf;
    }

	public bool IsShowingFirstItem()
    {
        if (items.Count <= 0)
            return false;

        //return items[0].gameObject.active;//&& !items[0].Clipped;

		return items[0].gameObject.activeSelf;
    }
	...
	public void ClearList(bool destroy)
	{
		...
		if (destroy && gameObject.activeSelf == true)
		{
